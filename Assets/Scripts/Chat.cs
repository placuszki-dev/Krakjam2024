...
namespace WebglExample
{
    public class Chat : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            Connection.OnYouJoinedGame.AddListener((update) => { IncomingChat.text += "<br>" + "you joined the game."; });
            Connection.OnNewPlayerJoinGame.AddListener((player) => { IncomingChat.text += "<br>" + string.Format("{0} joined the game.", player.Name); });
            Connection.OnAPlayerLeftGame.AddListener((id) => { IncomingChat.text += "<br>" + "a player left the game."; });
        }
    }
}